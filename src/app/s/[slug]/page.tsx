  ...
  useEffect(() => {
    ...
    fetch(`/api/storefront/${slug}`)
      ...
      .then((data) => { 
        ...
        const publishedProducts = (data.products || []).filter((p: Product) => p.status === "active" || p.status === "Published");
...
function StoreHeaderTemplates({
  ...
  return <div className="bg-white pb-1"><div className="mx-auto max-w-5xl px-4"><div className="relative mt-0 h-[130px] overflow-hidden rounded-b-2xl sm:h-[160px]">{seller.bannerUrl ? <SafeImg src={seller.bannerUrl} alt={`${seller.name} banner`} className="h-full w-full object-cover" fallback={<div className={`flex h-full items-center justify-center bg-gradient-to-br ${theme.gradient}`}><span className="text-2xl font-bold text-white/80">{seller.name}</span></div>} /> : <div className={`flex h-full items-center justify-center bg-gradient-to-br ${theme.gradient}`}><span className="text-2xl font-bold text-white/80">{seller.name}</span></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/10" /><div className="absolute bottom-3 left-3 right-3 flex items-center gap-3"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl border-2 border-white/80 bg-white shadow-lg sm:h-16 sm:w-16">{seller.logoUrl ? <SafeImg src={seller.logoUrl} alt={seller.name} className="h-full w-full object-cover" fallback={<AvatarPlaceholder name={seller.name} className="h-full w-full text-lg" />} /> : <AvatarPlaceholder name={seller.name} className="h-full w-full text-lg" />}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h1 className="truncate text-lg font-bold text-white drop-shadow-md">{seller.name}</h1>{isVerified && <span title={t.verified}><BadgeCheck className="h-4 w-4 shrink-0 text-green-400 drop-shadow" /></span>}</div><div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-xs text-white/80">{ratingInfo.count > 0 && <span className="flex items-center gap-1"><Star className="h-3 w-3 fill-amber-400 text-amber-400" /><span className="font-semibold text-white">{ratingInfo.avg}</span><span>({ratingInfo.count})</span></span>}{seller.city && <span className="flex items-center gap-0.5"><MapPin className="h-3 w-3" />{seller.city}</span>}{seller.businessType && seller.businessType !== "Retail" && <span className="inline-flex items-center gap-0.5 rounded-full bg-white/20 px-1.5 py-0.5 text-[10px] font-medium text-white backdrop-blur-sm">{seller.businessType}</span>}</div></div><div className="hidden sm:flex items-center gap-1">{social.whatsapp && <a href={social.whatsapp} target="_blank" rel="noreferrer" className="rounded-lg p-2 text-white/70 hover:bg-white/20 hover:text-white transition" title="WhatsApp"><MessageCircle className="h-4 w-4" /></a>}<button onClick={() => { if (navigator.share) navigator.share({ url: window.location.href, title: seller.name }); else { navigator.clipboard.writeText(window.location.href); } }} className="rounded-lg p-2 text-white/70 hover:bg-white/20 hover:text-white transition" title={t.shareStore}><Share2 className="h-4 w-4" /></button></div></div></div></div><div className="mt-2 px-2 sm:px-4">{stats}</div></div></div>;
}
