  ...
  return (
    <div className="bg-white pb-1">
      <div className="mx-auto max-w-5xl px-4">
        {/* Banner */}
        <div className="relative mt-0 h-[130px] overflow-hidden rounded-b-2xl sm:h-[160px]">
          {seller.bannerUrl ? (
            <SafeImg
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center gap-3">
            <div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl border-2 border-white/80 bg-white shadow-lg sm:h-16 sm:w-16">
              {seller.logoUrl ? (
                <SafeImg
                  src={seller.logoUrl}
                  alt={seller.name}
                  className="h-full w-full object-cover"
                  fallback={<AvatarPlaceholder name={seller.name} className="h-full w-full text-lg" />}
                />
              ) : (
                <AvatarPlaceholder name={seller.name} className="h-full w-full text-lg" />
              )}
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {isVerified && (
                  <span title={t.verified}>
                    <BadgeCheck className="h-4 w-4 shrink-0 text-green-400 drop-shadow" />
                  </span>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-xs text-white/80">
                {ratingInfo.count > 0 && (
                  <span className="flex items-center gap-1">
                    ...
                    <span className="font-semibold text-white">{ratingInfo.avg}</span>
                ...
                {seller.businessType && seller.businessType !== "Retail" && (
                  <span className="inline-flex items-center gap-0.5 rounded-full bg-white/20 px-1.5 py-0.5 text-[10px] font-medium text-white backdrop-blur-sm">
                    {seller.businessType}
                  </span>
                )}
              </div>
            ...
            <div className="hidden sm:flex items-center gap-1">
              {social.whatsapp && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg p-2 text-white/70 hover:bg-white/20 hover:text-white transition"
                  title="WhatsApp"
                >
                  <MessageCircle className="h-4 w-4" />
                </a>
              ...
              <button
                onClick={() => {
                  if (navigator.share) navigator.share({ url: window.location.href, title: seller.name });
        ...
        {/* Stats below banner */}
        <div className="mt-2 px-2 sm:px-4">{stats}</div>
      </div>
    </div>
  );
}
