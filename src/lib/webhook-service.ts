...
export const WEBHOOK_EVENTS = {
  ORDER_CREATED: OrderEvents.CREATED,
  ORDER_PAID: OrderEvents.PAID,
  ORDER_CONFIRMED: OrderEvents.CONFIRMED,
  ORDER_SHIPPED: OrderEvents.SHIPPED,
  ORDER_DELIVERED: OrderEvents.DELIVERED,
  ORDER_CANCELLED: OrderEvents.CANCELLED,
  ORDER_REFUNDED: OrderEvents.REFUNDED,
  ORDER_UPDATED: 'order.updated'
} as const;

// Helper function for sending order webhooks (backward compatibility)
export async function sendOrderWebhook(userId: number, eventType: string, data: Record<string, any>) {
  try {
    // Get seller/store info from user ID
    const storeInfo = await sql`
      SELECT s.id as store_id, s.seller_id
      FROM stores s
      WHERE s.user_id = ${userId}
      LIMIT 1
    `;
    ...
    await WebhookService.fireWebhooks({
      type: eventType,
      data,
      storeId: storeInfo[0].store_id,
      sellerId: storeInfo[0].seller_id
    });
  } catch (error) {
    console.error("Error sending order webhook:", error);
  }
